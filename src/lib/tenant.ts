...
/**
 * 直接在 app 上注册, 不走 fastify.register() 避免 plugin encapsulation
 * (Fastify 5 里 register 的 plugin 的 decorator/hook 只在自己作用域可见)
 */
export function registerTenant(app: FastifyInstance): void {
  // Fastify 5 breaking change: decorateRequest 第二个参数用 null (不是 '')
